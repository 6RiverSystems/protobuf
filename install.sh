...
apt-get install -y curl ninja-build clang 
...
/tmp/semantic-release -slug 6RiverSystems/protobuf  -branch_env -noci -nochange -flow -vf
VERSION="$(cat .version)"
...
fpm -s dir -t deb \
   -n protobuf-six-river --version ${VERSION} /usr/local/bin=/usr/local/bin
else
fpm -s dir -t deb \
   -n protobuf-six-river --version ${VERSION} /usr/local/bin=/usr/local/bin
fi
ls -la
pwd
export ARTIFACT_DEB_NAME="protobuf-six-river_${VERSION}_${ARCHITECTURE}.deb"
export ARTIFACTORY_DEB_NAME="protobuf-six-river_${VERSION}_${DISTRO}_${ARCHITECTURE}.deb"

time curl \
	-H "X-JFrog-Art-Api: ${ARTIFACTORY_PASSWORD}" \
	-T "${WORKSPACE}/artifacts/${ARTIFACT_DEB_NAME}" \
	"https://sixriver.jfrog.io/sixriver/debian/pool/main/c/protobuf-sixriver/${ARTIFACTORY_DEB_NAME};deb.distribution=${DISTRO};deb.component=main;deb.architecture=${ARCHITECTURE}"
